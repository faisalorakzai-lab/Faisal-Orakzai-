...
function requireDriverAuth(
  authHeader: string | undefined
): { claims: { sub: string; exp: number; role: string } } | { error: string; status: number } {
  ...
  try {
    ...
    const payload = JSON.parse(Buffer.from(parts[1], "base64").toString("utf8")) as {
      sub?: unknown; exp?: unknown; iss?: unknown; role?: unknown;
    };
    ...
    if (typeof payload.sub !== "string" || typeof payload.exp !== "number")
                                          return { error: "Unauthorized — malformed payload",          status: 401 };
...
// POST /api/otc/driver/otp-request
// Checks driver exists in Supabase, generates OTP (demo: returned in response).
...
// POST /api/otc/driver/otp-verify
// Verifies OTP and issues a driver JWT.
...
// PATCH /api/otc/driver/toggle
// Sets driver is_online status.
router.patch("/driver/toggle", async (req, res) => {
  ...
  req.log.info({ driverId, is_online }, "Driver toggled online status");
  res.json({ is_online });
});

// GET /api/otc/driver/pending
// Returns the latest ride_request assigned to this driver that still needs a response.
router.get("/driver/pending", async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin
    .from("ride_requests")
    .select("id, pickup_address, dropoff_address, total_fare, distance_km, ride_type, payment_method")
    .eq("driver_id", driverId)
    .eq("status", "assigned")
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (error) { res.status(500).json({ error: "Failed to query rides" }); return; }
  res.json({ ride: data ?? null });
});

// PATCH /api/otc/driver/request/:id/respond
// Driver accepts or declines an assigned ride.
router.patch("/driver/request/:id/respond", async (req, res) => {
  ...
  const driverId = auth.claims.sub;
  const newStatus = action === "accept" ? "driver_accepted" : "searching";
  ...
  req.log.info({ rideId: id, driverId, action }, "Driver responded to ride request");
